...
namespace SnowChess.Pieces
{
    ...
    {
        public Queen(char letter, int number, int side) : base(letter, number, side)
        {
        }
        ...
        {
            // this is very ugly, but since a queen is basically a Rook + Bishop, that's what I did.
            ...
            // right
            for (char j = (char)(Letter + 1); j < 'i'; j++)
            {
                bool result = CheckSquare(j, Number, ref moves);

                if (!result) break;
            }

            // left
            for (char j = (char)(Letter - 1); j >= 'a'; j--)
            {
                bool result = CheckSquare(j, Number, ref moves);
